...
def assign_boxes_to_classes(bounding_boxes, classes, scores):
    ...
    for each_box, each_class, each_score in zip(bounding_boxes, classes, scores):
        if each_score >= 0.05:
            boxes_to_classes[each_class.item()].append(
                np.array([each_box[0], each_box[1], each_box[2], each_box[3], each_score.item(), each_class.item()]))
...
def confluence_nms(bounding_boxes, scores, classes, confluence_thr, gaussian=False, score_thr=0.05, sigma=0.5):
    """
    Parameters:
       bounding_boxes: list of bounding boxes (x1,y1,x2,y2)
       scores: list of class confidence scores (0.0-1.0)
...
def confluence(bounding_boxes, scores, classes, confluence_thr, gaussian=False, score_thr=0.05, sigma=0.5):
    """
    Parameters:
       bounding_boxes: list of bounding boxes (x1,y1,x2,y2)
       classes: list of class identifiers (int value, e.g. 1 = person)
    ...
    for each_class in class_mapping:
        ...
        while dets.size > 0:
            ...
            while len(confluence_scores) < np.size(dets, 0):
                ...
                xx1, yy1, xx2, yy2, cconf = dets[np.arange(len(dets)) != current_box, 0], dets[
                    np.arange(len(dets)) != current_box, 1], dets[np.arange(len(dets)) != current_box, 2], dets[
                                                np.arange(len(dets)) != current_box, 3], dets[
                                                np.arange(len(dets)) != current_box, 4]
                min_x, min_y, max_x, max_y = np.minimum(x1, xx1), np.minimum(y1, yy1), np.maximum(x2, xx2), np.maximum(
                    y2, yy2)
                x1, y1, x2, y2 = normalise_coordinates(x1, y1, x2, y2, min_x, max_x, min_y, max_y)
