...
from .losses import IOUloss, AsaNWDLoss
from .network_blocks import BaseConv, DWConv, SpikeConv, SpikeConvWithoutBN
...
class SpikeYOLOXHead(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        self.strides = strides
        self.grids = [torch.zeros(1)] * len(in_channels)
        self.nwd_loss = AsaNWDLoss(reduction="none")

    def initialize_biases(self, prior_prob):
        for conv in self.cls_preds:
            b = conv.conv.bias.view(1, -1)
            b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
            conv.conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                output = torch.cat([reg_output, obj_output, cls_output], 2)
                ...
                output = torch.cat(
                    [reg_output, obj_output.sigmoid(), cls_output.sigmoid()], 2
                )
                output.mean(0)
            outputs.append(output)
        ...
        if self.training:
            ...
            # [batch, n_anchors_all, 85]
            outputs = torch.cat(
                [x.mean(0).flatten(start_dim=2) for x in outputs], dim=2
            ).permute(0, 2, 1)
            if self.decode_in_inference:
                return self.decode_outputs(outputs, dtype=xin[0].type())
            else:
    ...
    def get_losses(
        ...
        loss_nwd = (
           self.nwd_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)
        ).sum() / num_fg
        loss_obj = (
            self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)
        ).sum() / num_fg
        ...
        reg_weight = 0.0
        reg_weight1 = 8.0
        loss = reg_weight * loss_iou + loss_obj + loss_cls + loss_l1 + reg_weight1 * loss_nwd
        #loss = reg_weight * loss_iou + loss_obj + loss_cls + loss_l1

        return (
            loss,
            reg_weight * loss_iou,
            reg_weight1 * loss_nwd,
            loss_obj,
            loss_cls,
            loss_l1,
            num_fg / max(num_gts, 1),
